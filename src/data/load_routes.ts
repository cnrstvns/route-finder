import '@/lib/env';
import { db, airport, route as routeTable } from '@/db';
import { eq, and } from 'drizzle-orm';
import fetch from 'node-fetch';
import { airlines } from './load_airlines';

(async () => {
  for (const [_, airline] of airlines) {
    const response = await fetch(
      `https://www.flightsfrom.com/api/airline/${airline}`,
      {
        headers: {
          ...
          Referer: `https://www.flightsfrom.com/${airline}`,
    ...
    for (const route of routes) {
      ...
      if (!existingRoute.length) {
        ...
        await db.insert(routeTable).values({
          ...
          aircraftCodes: route.aircraft_codes.replace(' ', ''),
